  ...
  var Pizza = {
      ...
      if (settings.mode !== 'circle'){
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (settings.mode === 'donut-path'){
            self = this;
            (function (i, settings, svg, cx, cy, xCenter, yCenter, visible_text, r) {
              setTimeout(function () {
                self.drawDonutPath.apply(self,[i,settings, svg, cx, cy, xCenter, yCenter, visible_text, r])  
              }, 1); 
            })(i, settings, svg, cx, cy, xCenter, yCenter, visible_text, r);
          }
    ...
    drawDonutPath: function (i, settings, svg, cx, cy, xCenter, yCenter, visible_text, r) {
        ...
        if (i === 0){
          ...
          var centerText = settings.text.split(' ');

          var txtMain = this.checkExistence({
            svg: svg,
            id: 'main-circle',
            type: 'text',
            params: [cx-(radius/1.5),cy+(radius/3), centerText[0]]
          });
          ...
          var txtMain2 = this.checkExistence({
            ...
            id: 'main-text2',
            type: 'text',
            params: [cx-(radius/1.5)+(radius/20),cy+(radius/1.75), centerText[1]]
          });
